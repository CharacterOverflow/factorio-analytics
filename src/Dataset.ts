// Common variables and functions for all datasets
// most functions will be overridden here, but this provides a common usage pattern
...
export class Dataset {
    ...
    // All of these will always be set - because we can only make a dataset from a finished trial
    startedAt: Date
    endedAt: Date
    trial: Trial

    // file path to the raw data file, or for system data, the entire raw dataset string
    raw?: string

    // METADATA

    // average <thing> per MINUTE. each field represents an item, value is the per-MINUTE rate
    avg: any

    // total <thing> produced. each field represents an item, value is the total produced
    total: any
...
export class ItemDataset extends Dataset {
    ...
    async parseData(): Promise<IGameFlowTick[]> {
        // load the file in 'raw' and parse it into the dataset
        const raw = await fs.readFile(this.raw, 'utf-8');
...
export class ElectricDataset extends Dataset {
    ...
    async parseData(): Promise<IGameElectricTick[]> {
        const raw = await fs.readFile(this.raw, 'utf-8');
...
export class CircuitDataset extends Dataset {
    ...
    async parseData(): Promise<IGameCircuitTick[]> {
        let raw = await fs.readFile(this.raw, 'utf-8');
...
export class PollutionDataset extends Dataset {
    ...
    async parseData(): Promise<IGamePollutionTick[]> {
        let raw = await fs.readFile(this.raw, 'utf-8');
...
export class SystemDataset extends Dataset {
    ...
    async parseData(): Promise<ISystemTick[]> {
